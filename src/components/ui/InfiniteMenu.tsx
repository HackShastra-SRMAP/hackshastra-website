...
function createProgram(gl: WebGL2RenderingContext, shaderSources: string[], _transformFeedbackVaryings?: string[] | null, attribLocations?: Record<string, number>) {
...
class InfiniteGridMenu {
  ...
  constructor(canvas: HTMLCanvasElement, items: MenuItem[], onActiveItemChange: (index: number) => void, onMovementChange: (isMoving: boolean) => void, onInit: ((sk: InfiniteGridMenu) => void) | null = null, scale = 1.0) {
    this.canvas = canvas;
    this.items = items || [];
  ...
  init(onInit: ((sk: InfiniteGridMenu) => void) | null) {
    ...
    this.discBuffers = { vertices: this.discGeo.vertexData, indices: this.discGeo.indexData, uvs: this.discGeo.uvData };
  ...
  animate(_deltaTime: number) {
    const gl = this.gl;
    this.control.update(_deltaTime, this.TARGET_FRAME_DURATION);
    ...
    positions.forEach((p, ndx) => {
      ...
      mat4.multiply(matrix, matrix, mat4.targetTo(mat4.create(), [0, 0, 0], p as [number, number, number], [0, 1, 0]));
  ...
  updateCameraMatrix() {
    mat4.targetTo(this.camera.matrix, this.camera.position as [number, number, number], [0, 0, 0], this.camera.up as [number, number, number]);
